...
impl<'a, C: Configuration> LineParser<'a, C> {
    ...
    fn new_peer(value: &str) -> Result<ParserState, ConfigError> {
        ...
        let k: [u8; SIZE_STATIC_KEM_PUB_KEY] = <[u8; SIZE_STATIC_KEM_PUB_KEY]>::try_from(pkv)
            .map_err(|_| ConfigError::InvalidHexValue)?;
        let pk_pq: [u8; SIZE_STATIC_KEM_PUB_KEY] =
            k[..].try_into().map_err(|_| ConfigError::InvalidHexValue)?;
        ...
        match self.state {
            ...
            ParserState::Interface => match key {
                ...
                "private_key" => match Vec::from_hex(value) {
                    Ok(skv) => {
                        let k =
                            <[u8; SIZE_STATIC_KEM_SECRET_KEY + SIZE_STATIC_KEM_PUB_KEY]>::try_from(
                                skv,
                            )
                            .map_err(|_| ConfigError::InvalidHexValue)?;

                        self.config.set_private_key(
                            if k.ct_eq(&[0u8; SIZE_STATIC_KEM_SECRET_KEY + SIZE_STATIC_KEM_PUB_KEY])
                                .into()
                            {
                                None
                            } else {
                                let sk_pq: [u8; SIZE_STATIC_KEM_SECRET_KEY] =
                                    k[..SIZE_STATIC_KEM_SECRET_KEY].try_into().unwrap();
                                let pk_pq: [u8; SIZE_STATIC_KEM_PUB_KEY] =
                                    k[SIZE_STATIC_KEM_SECRET_KEY..].try_into().unwrap();
