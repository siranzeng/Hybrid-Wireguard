...
use crate::wireguard_hybrid_new::handshake::crypto_params::{
    HASH, MAC, SIZE_HASH, SIZE_SESSION_ID, SIZE_TAG, XOPEN, XSEAL,
};
use blake2::Blake2s;
...
const LABEL_COOKIE_PLAIN: &[u8] = b"hybrid-wireguard-cookie-v1";
...
const COOKIE_SECRET_WINDOW_SECS: u64 = 60;
...
fn addr_to_mac_bytes(addr: &SocketAddr) -> Vec<u8> {
    match addr {
        SocketAddr::V4(addr) => {
            ...
            res.extend(&addr.port().to_be_bytes());
            res
        }
        SocketAddr::V6(addr) => {
...
fn now_window() -> u64 {
    let now = SystemTime::now()
        ...
        .as_secs();
    (now / COOKIE_SECRET_WINDOW_SECS) * COOKIE_SECRET_WINDOW_SECS
}

fn window_secret(root: &[u8; SIZE_SECRET], window: u64) -> [u8; SIZE_SECRET] {
    HASH!(root, &window.to_be_bytes())
}

fn cookie_plain(root: &[u8; SIZE_SECRET], src: &[u8], window: u64) -> [u8; SIZE_COOKIE] {
    let secret = window_secret(root, window);
    MAC!(&secret, src, LABEL_COOKIE_PLAIN)
}
...
impl Generator {
    ...
    pub fn new(psk: &Psk, responder_pk_hash: &[u8; SIZE_HASH]) -> Generator {
        Generator {
            mac1_key: HASH!(LABEL_MAC1, psk).into(),
            cookie_key: HASH!(LABEL_COOKIE, responder_pk_hash).into(),
            last_mac1: None,
            cookie: None,
        }
    }

    pub fn set_mac1_key(&mut self, psk: &Psk) {
        self.mac1_key = HASH!(LABEL_MAC1, psk);
        self.last_mac1 = None;
        self.cookie = None;
    }
...
pub struct Validator {
    ...
    cookie_root_secret: [u8; SIZE_SECRET],
}

impl Validator {
    pub fn new(
        psk: &Psk,
        responder_pk_hash: &[u8; SIZE_HASH],
        cookie_root_secret: &[u8; SIZE_SECRET],
    ) -> Validator {
        Validator {
            mac1_key: HASH!(LABEL_MAC1, psk).into(),
            cookie_key: HASH!(LABEL_COOKIE, responder_pk_hash).into(),
            cookie_root_secret: *cookie_root_secret,
        }
    }
    ...
    pub fn new_device(
        ...
        Validator::new(&[0u8; 32], responder_pk_hash, cookie_root_secret)
    }
    ...
    fn get_current_tau(&self, src: &[u8]) -> [u8; SIZE_COOKIE] {
        cookie_plain(&self.cookie_root_secret, src, now_window())
    }

    fn check_tau(&self, src: &[u8], candidate: &[u8; SIZE_COOKIE]) -> bool {
        let current = now_window();
        let previous = current.saturating_sub(COOKIE_SECRET_WINDOW_SECS);
        let current_tau = cookie_plain(&self.cookie_root_secret, src, current);
        let previous_tau = cookie_plain(&self.cookie_root_secret, src, previous);
        bool::from(current_tau.ct_eq(candidate)) || bool::from(previous_tau.ct_eq(candidate))
    }

    pub fn create_cookie_reply<R: RngCore + CryptoRng>(
        &self,
        rng: &mut R,
        receiver: &SessionId,  // session id of incoming message
        ...
        msg.f_receiver.copy_from_slice(receiver);
        rng.fill_bytes(&mut msg.f_nonce);
        XSEAL!(
            &self.cookie_key,            // key
            &msg.f_nonce,                // nonce
            &macs.f_mac1,                // ad
            &self.get_current_tau(&src), // pt
    ...
    pub fn check_mac2(&self, inner: &[u8], src: &SocketAddr, macs: &MacsFooter) -> bool {
        ...
        let current_tau = cookie_plain(&self.cookie_root_secret, &src, current);
        let previous_tau = cookie_plain(&self.cookie_root_secret, &src, previous);
        bool::from(MAC!(&current_tau, inner, macs.f_mac1).ct_eq(&macs.f_mac2))
            || bool::from(MAC!(&previous_tau, inner, macs.f_mac1).ct_eq(&macs.f_mac2))
    }
}
...
mod tests {
    ...
    fn new_validator_generator() -> (Validator, Generator) {
        ...
        let cookie_root = HASH!(b"test-cookie-root", &psk);
        let responder_pk_hash = HASH!(b"test-responder", &psk);

        (
            Validator::new(&psk, &responder_pk_hash, &cookie_root),
            Generator::new(&psk, &responder_pk_hash),
        )
    }

    proptest! {
        #[test]
        fn test_cookie_reply(inner1 : Vec<u8>, inner2 : Vec<u8>, receiver : [u8; SIZE_SESSION_ID]) {
