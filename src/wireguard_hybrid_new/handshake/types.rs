...
impl fmt::Display for HandshakeError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            HandshakeError::CookieRequired => {
                write!(f, "Cookie challenge required for this source")
            }
            HandshakeError::InvalidCookie => write!(f, "Cookie validation failed"),
            HandshakeError::InvalidTi => write!(f, "Initiation t_i field failed integrity check"),
            HandshakeError::InvalidTr => write!(f, "Response t_r field failed integrity check"),
            HandshakeError::InvalidMode => write!(f, "Invalid Hybrid-WireGuard v2.3 mode"),
            HandshakeError::InvalidEpoch => write!(f, "Invalid Hybrid-WireGuard v2.3 epoch"),
            HandshakeError::InvalidRatchetKid => write!(f, "Invalid response KEM ratchet kid"),
            HandshakeError::RatchetStateMissing => write!(f, "Required ratchet state is missing"),
            HandshakeError::InvalidConfirm => write!(f, "Encrypted response confirmation failed"),
        }
    }
}
