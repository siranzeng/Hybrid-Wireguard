...
use crate::wireguard_hybrid_new::handshake::crypto_params::SIZE_HASH;
...
pub trait Configuration {
    ...
    fn set_ri_key(&self, ri_key: Option<[u8; 8]>);

    fn get_ri_key(&self) -> Option<[u8; 8]>;
...
impl<T: tun::Tun, B: udp::PlatformUDP> Configuration for WireGuardConfig<T, B> {
    ...
    fn set_private_key(
        &self,
        sk: Option<(StaticSecret, oqs::kem::SecretKey, oqs::kem::PublicKey)>,
    ) {
    ...
    fn set_ri_key(&self, ri_key: Option<[u8; 8]>) {
        #[cfg(feature = "hybrid_new")]
        {
            self.lock().wireguard.set_ri_key(ri_key);
        }
        #[cfg(not(feature = "hybrid_new"))]
        {
            let _ = ri_key;
    ...
    fn get_ri_key(&self) -> Option<[u8; 8]> {
        #[cfg(feature = "hybrid_new")]
        {
            self.lock().wireguard.get_ri_key()
        }
        #[cfg(not(feature = "hybrid_new"))]
        {
            None
